...
public final class GameState<G extends Game<G>> {
    ...
    private final IMinestomModule owner;
    private final String name;

    private final ISimpleLogger logger;

    private final StateTickTimer timer;
    ...
    private final ObjectList<PhasedEventContainer<G>> listeners;

    private final ObjectList<Task<G>> activeTasks;
    private final ObjectList<PhasedEventContainer<G>> activeListeners;

    private volatile int phaseIdx;

    @SuppressWarnings("unchecked")
    GameState(final IMinestomModule owner, final String name, final GameProvider<G> provider) {
        ...
        this.timer = new StateTickTimer(this);
        this.stateInvoker = new SimpleInstanceInvoker(owner.invoker());
    ...
    public final <T extends Task<G>> T task(Class<T> taskType) {
        return tasks.stream().filter(task -> taskType.isAssignableFrom(task.get().getClass())).findFirst()
            .map(task -> taskType.cast(task.get())).orElse(null);
    }
    ...
    public final void terminate() {
        ...
        timer.stop();
        for (PhasedEventContainer<G> listener : activeListeners) {
            listener.unregister();
        }
        for (Task<G> task : activeTasks) {
            task.onStop(this);
